...
export function UserAdmin({ currentUser }: { currentUser: User }) {
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      setLoadError(cause instanceof Error ? cause.message : 'Cannot load users')
    } finally {
      setLoading(false)
    }
  }, [])
  useEffect(() => {
    ...
    request<User[]>('/api/users')
      ...
      .catch((cause: unknown) => {
        if (active)
          setLoadError(
            cause instanceof Error ? cause.message : 'Cannot load users',
          )
      })
      .finally(() => {
  ...
  return (
    <main className="app admin-page">
      ...
      {loadError && (
        <div role="alert">
          <p className="add-form-error">{t(loadError ?? '')}</p>
          <button
      ...
      <ul className="roster" aria-label={t('Users')}>
        {users.map((user) => (
          <UserRow
            key={user.id}
            user={user}
...
function CreateUser({ onSaved }: { onSaved: () => Promise<void> }) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    if (
      ...
      setMessage(t('User {name} created', { name: login }))
      setLogin('')
      setPassword('')
      setRole('user')
      await onSaved()
  ...
  return (
    <form className="add-form" onSubmit={submit}>
      <h2>{t('Add user')}</h2>
      <label htmlFor="new-login">{t('Login')}</label>
      <input
      ...
      <label htmlFor="new-password">
        {t('Password (at least 3 characters)')}
      </label>
      <input
        className="add-form-input"
        id="new-password"
      ...
      <button className="button" disabled={pending}>
        {pending ? t('Creating…') : t('Create user')}
      </button>
      {error && (
      ...
      {message && (
        <p role="status" className="add-form-note">
          {t(message ?? '')}
        </p>
      )}
    </form>
...
function UserRow({
  ...
  const { pending, error, run } = useAsyncAction()
  return (
    <li className="roster-row user-row">
      ...
      {editing && (
        <form
          ...
          onSubmit={async (event) => {
            ...
            if (
              await run(() =>
                request<User>(`/api/users/${user.id}`, {
                  method: 'PATCH',
                  headers: jsonHeaders,
              ...
              setPassword('')
              setEditing(false)
              setMessage('Saved. Previous sessions have been signed out.')
        ...
        >
          ...
          <label htmlFor={`password-${user.id}`}>
            {t('New password (leave empty to keep)')}
          </label>
          <input
            className="add-form-input"
            type="password"
          ...
          />
          <p className="add-form-note">
            {t('Saving changes signs this user out of all sessions.')}
            {self ? t(' You will need to sign in again.') : ''}
          </p>
