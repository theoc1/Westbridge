...
export function Phonebook({ book, disabled }: { book: ReturnType<typeof usePhonebook>; disabled: boolean }) {
  ...
  return <aside className="phonebook" aria-label="Personal phonebook">
    ...
    {book.loading ? <p role="status">Loading phonebook…</p> : book.contacts.length === 0 ? <p className="empty-state">Your phonebook is empty.</p> : <>
      ...
      <ul className="contact-list" aria-label="Contacts">{book.contacts.map(contact => <li key={contact.id} className="contact-row">
        <label className="contact-identity"><input type="checkbox" aria-label={`Select ${contact.name}`} disabled={calling} checked={selected.has(contact.id)} onChange={e => { const checked = e.target.checked; setSelected(previous => { const next = new Set(previous); if (checked) next.add(contact.id); else next.delete(contact.id); return next }) }} /><span><strong>{contact.name}</strong><span className="roster-number">{contact.number}</span></span></label>
        <div className="contact-actions">{deleting === contact.id ? <><button className="button button-danger" disabled={pending} onClick={() => { void remove(contact.id) }}>Delete?</button><button className="button button-quiet" disabled={pending} onClick={() => setDeleting(null)}>Cancel</button></> : <><button className="button button-quiet" aria-label={`Edit ${contact.name}`} disabled={pending || calling} onClick={() => open(contact)}>Edit</button><button className="button button-quiet" aria-label={`Delete ${contact.name}`} disabled={pending || calling} onClick={() => { setDeleting(contact.id); setEditor(null); setError(null) }}>Delete</button></>}</div>
